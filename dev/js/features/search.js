...
if (typeof PredictiveSearch === "undefined") {
  class PredictiveSearch extends HTMLElement {
    ...
    displayResults(results) {
      ...
      if (results.length === 0) {
        this.searchResults.innerHTML = '<p class="predictive-search__no-results">No results found</p>';
        this.openResults();
        return;
      }
      ...
      const resultsHTML = results
        .map(
          (result, index) => `
        <a href="${result.url}" class="predictive-search__result-item" data-index="${index}">
          <div class="predictive-search__result-image">
            ${result.featured_image ? `<img src="${result.featured_image}" alt="${result.title}">` : ""}
          </div>
    ...
    selectNext() {
      const items = this.searchResults?.querySelectorAll(".predictive-search__result-item");
    ...
    updateSelection(items) {
      items.forEach((item, index) => {
        item.classList.toggle("predictive-search__result-item--selected", index === this.selectedIndex);
      });
