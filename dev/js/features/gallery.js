...
if (typeof MediaGallery === "undefined") {
  class MediaGallery extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.thumbnails = Array.from(this.querySelectorAll(".media-gallery__thumbnail"));
    ...
    setupZoom() {
      ...
      this.mainImage.addEventListener("mouseleave", () => {
        this.zoomEnabled = false;
        this.mainImage.classList.remove("media-gallery__main-image--zoom-enabled");
      });
    ...
    selectImage(index) {
      ...
      const newImageSrc = selectedThumbnail.dataset.fullImage || selectedThumbnail.src;
    ...
    previousImage() {
      const newIndex = this.currentIndex > 0 ? this.currentIndex - 1 : this.thumbnails.length - 1;
      this.selectImage(newIndex);
    }

    nextImage() {
      const newIndex = this.currentIndex < this.thumbnails.length - 1 ? this.currentIndex + 1 : 0;
      this.selectImage(newIndex);
    }
