...
class CSSBundleOptimizer {
  ...
  async extractCriticalCSS() {
    ...
    try {
      const cssPath = path.join(this.assetsDir, 'style-base.css');
      const criticalPath = path.join(this.assetsDir, 'critical.css');

      if (!(await fs.pathExists(cssPath))) {
        throw new Error('style-base.css not found. Run CSS build first.');
      }
      ...
      const nonCriticalPath = path.join(this.assetsDir, 'non-critical.css');
      ...
      console.log(
        `📊 Critical CSS: ${(criticalCSS.length / 1024).toFixed(2)}KB`
      );
      console.log(
        `📊 Non-critical CSS: ${(nonCriticalCSS.length / 1024).toFixed(2)}KB`
      );
    } catch (error) {
  ...
  extractCriticalRules(css) {
    ...
    for (const line of lines) {
      ...
      const isCritical = criticalSelectors.some(
        (selector) =>
          trimmedLine.includes(selector) && !trimmedLine.startsWith('/*')
      );
  ...
  async generateOptimizationReport(originalCSS, criticalCSS, nonCriticalCSS) {
    const report = {
      ...
      savingsPercentage: (
        ((originalCSS.length - criticalCSS.length) / originalCSS.length) *
        100
      ).toFixed(2),
      recommendations: [],
    };
    ...
    if (criticalCSS.length > 50 * 1024) {
      ...
      report.recommendations.push(
        'Critical CSS is large, consider further optimization'
      );
    }
  ...
  async optimizeCSS() {
    ...
    try {
      const cssPath = path.join(this.assetsDir, 'style-base.css');
      const optimizedPath = path.join(this.assetsDir, 'style-optimized.css');

      if (!(await fs.pathExists(cssPath))) {
        throw new Error('style-base.css not found');
      }
      ...
      console.log(
        `📊 Savings: ${(savings / 1024).toFixed(2)}KB (${savingsPercentage}%)`
      );
    } catch (error) {
  ...
  async createCSSManifest() {
    ...
    try {
      ...
      const manifestPath = path.join(this.assetsDir, 'css-manifest.json');
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      console.log(
        'Usage: node css-bundle-optimizer.js [extract-critical|optimize|manifest]'
      );
  }
}
