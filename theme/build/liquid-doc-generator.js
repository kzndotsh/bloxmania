...
class LiquidDocGenerator {
  ...
  async parseSections() {
    ...
    const sectionFiles = glob.sync('sections/*.liquid', { cwd: this.options.inputDir });
  ...
  async parseSnippets() {
    ...
    const snippetFiles = glob.sync('snippets/*.liquid', { cwd: this.options.inputDir });
  ...
  async parseTemplates() {
    ...
    const templateFiles = glob.sync('templates/*.liquid', { cwd: this.options.inputDir });
...
${this.documentation.sections.map(section => `
...
${this.documentation.snippets.map(snippet => `
...
${this.documentation.templates.map(template => `
...
${this.documentation.assets.filter(a => a.type === 'js').map(asset => `
...
${this.documentation.assets.filter(a => a.type === 'css').map(asset => `
### [${asset.name}](${asset.name}.md)
${asset.description}
`).join('')}
`;
...
${this.documentation.config.map(config => `
...
`).join('') : 'No blocks available.'}

## Dependencies

${section.dependencies.length > 0 ? section.dependencies.map(dep => `
- **${dep.type}**: ${dep.name}
...
${snippet.parameters.length > 0 ? snippet.parameters.map(param => `
### ${param.name}
- **Type**: ${param.type}
...
${snippet.dependencies.length > 0 ? snippet.dependencies.map(dep => `
- **${dep.type}**: ${dep.name}
`).join('') : 'No dependencies.'}

## Usage

${snippet.usage.length > 0 ? snippet.usage.map(usage => `
\`\`\`liquid
${usage}
\`\`\`
`).join('') : 'No usage examples available.'}
...
${template.dependencies.length > 0 ? template.dependencies.map(dep => `
- **${dep.type}**: ${dep.name}
`).join('') : 'No dependencies.'}
