...
class CartNotification extends HTMLElement {
        ...
        <div class="cart-notification__footer">
          <div class="cart-notification__cart-info">
            <p class="cart-notification__cart-total">
              Cart total: ${this.formatPrice(cart.total_price)} (${cart.item_count} ${cart.item_count === 1 ? 'item' : 'items'})
            </p>
          </div>
          <div class="cart-notification__actions">
            <button class="cart-notification__continue btn btn--secondary" onclick="this.closest('cart-notification').close()">
              Continue shopping
            </button>
            <a href="/cart" class="cart-notification__view-cart btn btn--primary">
              View cart
            </a>
          </div>
        </div>
  ...
  open() {
    ...
    // Dispatch event
    this.dispatchEvent(new CustomEvent('cart-notification:opened', {
      bubbles: true,
  ...
  close() {
    ...
    // Dispatch event
    this.dispatchEvent(new CustomEvent('cart-notification:closed', {
      bubbles: true,
      detail: { notification: this }
    }));
  }
